  ...
  passport.authenticate("google", {
    ...
    if (!user) {
      if (info && info.message === "NOT_REGISTERED") {
        ...
        return res.redirect(`${process.env.FRONTEND_URL}/login.html?${params.toString()}`);
      }

      if (info && info.message === "NOT_A_PATIENT") {
        return res.redirect(`${process.env.FRONTEND_URL}/login.html?error=not_patient`);
      }

      return res.redirect(`${process.env.FRONTEND_URL}/login.html?error=not_registered`);
    }
    ...
    if (user.role === 'doctor') {
      return res.redirect(`${process.env.FRONTEND_URL}/doctor-dashboard.html?token=${token}&user=${userParam}`);
    }
    return res.redirect(`${process.env.FRONTEND_URL}/patient-dashboard.html?token=${token}&user=${userParam}`);
  })(req, res, next);
});
